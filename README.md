...
[Drone Delivery Challenge Prompt (PDF)](DroneDeliveryChallenge.pdf)
...
I had not used any of these libraries previously. I thought that this project seemed like a good opportunity to try out libraries. The project also uses some common libraries (e.g., JUnit, Mockito).
...
#### Arguments ####
**Required Arguments**:
Input filepath (*must be an absolute path*)
```
-i <INPUT_FILEPATH>
--input <INPUT_FILEPATH>
```

**Additional (optional) arguments & flags**:
...
The scheduler to use (which implementation of `DeliveryScheduler`):
```
-s <CLASS_NAME_OF_SCHEDULER>
--scheduler=<CLASS_NAME_OF_SCHEDULER>

Default:
--scheduler=PermutationOptimizingDeliveryScheduler
```

Example:
`$ ./gradlew run --args="--input=<PROJECT_ROOT>/src/main/resources/input/test-input-3 --scheduler=MinTransitTimeDeliveryScheduler"`
...
6A. Doesn't exceed 2GB  
6B. Is UTF-8 format  
6C. Contains only valid data (all order lines are in expected format)  - invalid input can be skipped using the `--exitOnInvalidInput` flag.  
6D. Assume orders are ordered from earliest order placement time to latest.  
...
1. Use the earliest order placed time. Given Assumption 6D, this would be the first order.
...
- [ ] Increase code coverage to 90% (currently 80%)
- [ ] Polling service:
...
- [ ] If a polling service exists, the `DeliveryScheduler` implementations need to be modified so that:
    * New orders are enqueued in `queuedOrders`
    * Reconsider deliveries that have been scheduled but have not yet departed.
        * To optimize NPS, we may need to schedule new order(s) ahead of these previously scheduled deliveries.
...
Entry point:
`main(args)`: `com.nrojiani.drone.cli.MainKt`
* passed args to `com.nrojiani.drone.cli.CommandLineApplication`, calls `run()`

Overview of `CommandLineApplication`: `run()`: (Via dependencies), processes the specified input file, schedules the deliveries and calculates NPS, and writes the output file.

TODO - add architecture diagram  
TODO - diagram of processing pipeline  
